...
export default function ArticleListPage() {
        ...
        try {
            const promises = Array.from(selectedIds).map(async (id) => {
                if (bulkAction === "delete") {
                    ...
                    return updateDoc(doc(db, "articles", id), { status: bulkAction });
                }
            });
    ...
    // Preview URL generator (matches article editor logic)
    const getPreviewUrl = (article: Article) => {
        if (!article.id || !article.category) return "";
        const categorySlug = slugify(article.category);
        const base = `https://www.psinv.net/en/articles/${categorySlug}`;
        if (categorySlug === 'area-guide' && article.city) {
            return `${base}/${slugify(article.city)}/${article.id}`;
        }
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-[#2d2d30]/60">
                            {currentArticles.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="px-6 py-8 text-center text-gray-500 text-[14px]">
                                        No articles found matching your search.
                                    </td>
                                </tr>
                            ) : (
                                currentArticles.map((article) => {
                                    const articlePreviewUrl = getPreviewUrl(article);
                                    return (
                                        <tr key={article.id} className={`hover:bg-[#28282c] transition-colors group ${selectedIds.has(article.id) ? 'bg-[#3c64f4]/5' : ''}`}>
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            if (articlePreviewUrl) {
                                                                setPreviewUrl(articlePreviewUrl);
                                                                setIsPreviewOpen(true);
                                                            }
                                                        }}
                                                        disabled={!articlePreviewUrl}
                                                        className={`p-2 rounded-md bg-[#2d2d30]/50 transition-colors ${articlePreviewUrl ? 'text-gray-500 hover:text-[#3c64f4] hover:bg-[#3c64f4]/10' : 'text-gray-700 cursor-not-allowed'}`}
                                                        title={articlePreviewUrl || 'No preview available'}
                                                    >
